...
export const controlPreventivoController = {
  historial: asyncHandler(async (req: Request, res: Response) => {
    ...
    res.json({ controles: controlPreventivoService.historialDeMascota(mascotaId) });
  }),

  proximosAVencer: asyncHandler(async (req: Request, res: Response) => {
    const dias = Number(req.query.dias) || 30;
    res.json({ controles: controlPreventivoService.proximosAVencer(dias) });
  }),

  crear: asyncHandler(async (req: Request, res: Response) => {
    const control = controlPreventivoService.crear(req.body);
