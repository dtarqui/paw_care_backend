...
//
//  - generarCobroQrBancario: SIEMPRE falla con un error claro hoy. No es un "si no
...
//  - verificarNotificacionWebhook: SÍ es funcional ya (comparación de secreto
...
export class ProveedorPagoQrNoConfiguradoError extends Error {
  constructor() {
    super(
      "El cobro por QR bancario no está disponible todavía en este entorno — falta conectar la API real del banco elegido (ver lib/pagoQr.ts)"
    );
    this.name = "ProveedorPagoQrNoConfiguradoError";
  }
}

export interface GenerarCobroQrInput {
  monto: number;
  /** Texto descriptivo para el banco (ej. "Atencion #123 - PawCare"), no un ID interno sensible. */
  referencia: string;
}

export interface CobroQrBancarioResultado {
 ...
 *   2. Hacer el POST real de generación de QR con el `input.monto`/`input.referencia`.
 *   3. Mapear la respuesta del banco a { referenciaExterna, qrPayload, expiraEn }.
 * Nada en `pagoQr.service.ts`, las rutas o el frontend necesita cambiar cuando esto
 * se implemente — ya están armados contra este contrato.
 */
export async function generarCobroQrBancario(
  _input: GenerarCobroQrInput
): Promise<CobroQrBancarioResultado> {
  throw new ProveedorPagoQrNoConfiguradoError();
}
 ...
 */
export function verificarNotificacionWebhook(secretoRecibido: string | undefined): boolean {
  const secretoEsperado = process.env.PAGO_QR_WEBHOOK_SECRET;
  if (!secretoEsperado || !secretoRecibido) return false;
  return secretoRecibido === secretoEsperado;
}
